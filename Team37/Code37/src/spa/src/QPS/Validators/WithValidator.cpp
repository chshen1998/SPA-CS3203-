...
TokenType WithValidator::validateRef(vector<PqlToken> refTokens)
{
    if (refTokens.size() == 1)
    {
	    ...
	    {
            throw SyntaxError("Invalid With clause parameters 1");
	    }
        return type;
    }
    else if (refTokens.size() == 3)
    {
        ...
        {
            throw SyntaxError("Invalid With clause parameters 2");
        }
        ...
        {
            throw SyntaxError("Invalid With clause parameters 3");
        }
        
        PqlToken attrName = refTokens[2];
        if (validAttrName.find(attrName.type) == validAttrName.end())
        {
            throw SyntaxError("Invalid With clause parameters 4");
        }
